...
**不以每次文件变动后立即提交。** 以一个完整特性或 bug 修复批次为单位。

触发方式：
1. 人工手动触发（向 Agent 说"提交"或"commit"）
2. Agent 触发前必须汇报当前 git 状态和变更摘要
3. 等待用户明确批准后执行
...
1. **安全问题扫描**：`git diff --cached` 检查是否有 `apiKey`、`token`、`password`、`secret` 等敏感关键词
